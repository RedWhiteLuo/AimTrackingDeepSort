...
from Match_IMG import IMG_match

class MultiDetection:
    def __init__(self):
        print("已使用目标跟踪器")
        self.tracks = []  # 这里保存到是track ["confirmed/unconfirmed", unique_id, age, [x,y,w,h], KM_predictor, if_miss, [imgs]]]
        self.detections = []  # 这里保存到是track [[x,y,w,h], conf, img]
        self.unique_id = 0

    def init_match(self, input_detections):  # 直接返回 tacked_list
        self.detections, result, offsets = [], [], 0  # 初始化
        [self.detections.append([detection[0], detection[1], detection[3]]) for detection in input_detections]
    ...
    def Matching_Cascade(self, detections_index, tracks_index):
        ...
        for T_index in tracks_index:
            ...
            for D_index in detections_index:
                ...
                cost_matrix.append(IMG_match(detection[2],track[6]))
    ...
    def Unmatched_Index_Perform(self, detections_index, tracks_index):
        ...
        for i in range(len(detections_index)):
            detection = detections[detections_index[i]]
            tracks.append(
                ["unconfirmed", self.unique_id, 4, detection[0], Kalman.Kalman(), False, [ detection[2] ] ] )
